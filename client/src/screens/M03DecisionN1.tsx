...
export function M03DecisionN1({ user }: { user: UserProfile }) {
  const [criteria, setCriteria] = useState({ cif: "", account: "", decision: "" });
  const [results, setResults] = useState<DecisionN1[]>([]);
  const [form, setForm] = useState<Partial<DecisionN1>>({});
  ...
  async function executeQuery() {
    ...
    try {
      ...
      const rows = await api.get<DecisionN1[]>(`/m03/search?${params.toString()}`);
      setResults(rows);
    } catch (e) {
  ...
  function selectRow(row: DecisionN1) {
    setForm(row);
    setDecisionInput(row.decision);
    setMotifInput(row.motif ?? "");
    setExpectedVersion(row.version);
    setActiveTab("main");
    api.get<any[]>(`/m03/${row.id}/audit`).then(setAudit).catch(() => setAudit([]));
  }
  ...
  async function save() {
    ...
    setError(null);
    try {
      const row = await api.put<DecisionN1>(`/m03/${form.id}`, {
  ...
  async function authorize() {
    ...
    setError(null);
    try {
      const row = await api.post<DecisionN1>(`/m03/${form.id}/authorize`, { actorLogin: user.login, actorRole: user.role });
      setForm(row);
      executeQuery();
    } catch (e) {
  ...
  async function remove() {
    ...
    setError(null);
    try {
      await api.del(`/m03/${form.id}?actorLogin=${user.login}&actorRole=${user.role}`);
      setForm({});
      executeQuery();
    } catch (e) {
  ...
  return (
    <EnterpriseShell
      functionId="BOA.DESH.N1.WORKLIST"
      screenTitle="M03 - Décision agence N1"
      user={user.login}
      toolbar={[
